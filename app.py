...
# Visual
# -----------------------------
st.set_page_config(page_title="REALHYPE OS", page_icon="⚡", layout="wide")
...
def get_users() -> Dict[str, Dict[str, str]]:
    try:
        ...
        pass
    # Local/demo fallback. Trocar antes de publicar.
    return {
        "socio1": {"password": "realhype123", "role": "partner", "name": "Sócio 1"},
        "socio2": {"password": "realhype123", "role": "partner", "name": "Sócio 2"},
        "sdr": {"password": "realhype123", "role": "sdr", "name": "SDR"},
    }
...
def login() -> bool:
    ...
    st.info("No teste local, usuários padrão: socio1, socio2 e sdr. Senha: realhype123. Troque nas Secrets antes de publicar.")
    return False
...
with st.sidebar:
    ...
    st.caption(f"Entrou como: **{DISPLAY_NAME}** · `{ROLE}`")
    page = st.radio("Menu", ["Visão Geral", "Leads & SDR", "Vendas", "Stock", "Financeiro", "Configurações"])
    st.divider()
    if st.button("Sair"):
...
<div class="rh-hero">
  ...
  <p class="rh-subtitle">Painel em tempo real para sócios e SDR: stock, Direct, encomendas, CTT e margem.</p>
  <span class="rh-chip">Portugal</span><span class="rh-chip">Direct</span><span class="rh-chip">CTT</span><span class="rh-chip">Pré-lançamento</span>
</div>
""", unsafe_allow_html=True)
st.write("")
...
if page == "Visão Geral":
    ...
    if low.empty:
        st.success("Nenhum produto abaixo do ponto de reposição.")
    else:
        st.warning("Produtos que precisam de atenção.")
    ...
    with tab1:
        with st.form("lead_form"):
            ...
            interest = st.selectbox("Interesse", ["Cordão fino", "Cordão médio", "Presença forte", "Pingente", "Presente", "Não definido"])
            stage = st.selectbox("Estágio", ["lead", "prospect", "cliente", "perdido"])
    ...
    with tab2:
        ...
        if not leads.empty:
            lead_id = st.selectbox("Lead", leads["id"].tolist(), format_func=lambda x: f"#{x} · {leads[leads.id==x].iloc[0]['name']}")
            new_stage = st.selectbox("Novo estágio", ["lead", "prospect", "cliente", "perdido"])
            if st.button("Atualizar estágio"):
    ...
    if products.empty:
        ...
        with st.form("order_form"):
            ...
            lead_id = st.selectbox("Lead/Cliente", lead_options, format_func=lambda x: "Sem lead vinculado" if x is None else f"#{x} · {leads[leads.id==x].iloc[0]['name']}")
            product_id = st.selectbox("Produto", products["id"].tolist(), format_func=lambda x: f"{products[products.id==x].iloc[0]['sku']} · {products[products.id==x].iloc[0]['name']} · Stock {products[products.id==x].iloc[0]['quantity']}")
            ...
            unit_price = st.number_input("Preço unitário (€)", min_value=0.0, value=default_price, step=0.5)
            confirmation = st.selectbox("Dupla confirmação CTT", ["pendente", "confirmado_texto", "confirmado_audio"])
            ...
            notes = st.text_area("Notas / morada resumida / observações")
            if st.form_submit_button("Registrar venda e dar saída no stock"):
                try:
                    ...
                    st.error(str(e))
    st.subheader("Últimas encomendas")
    orders = query_df("""
    ...
    with tab2:
        if not IS_PARTNER and ROLE != "sdr":
            ...
            with st.form("product_form"):
                ...
                category = st.selectbox("Categoria", ["Cordões Finos", "Cordões Médios", "Presença Forte", "Cordões Grossos", "Pingentes", "Outro"])
                qty = st.number_input("Quantidade", min_value=0, step=1, value=0)
                reorder = st.number_input("Ponto de reposição", min_value=0, step=1, value=1)
                cost = st.number_input("Custo unitário (BRL)", min_value=0.0, value=0.0, step=1.0)
                price = st.number_input("Preço venda (€)", min_value=0.0, value=0.0, step=0.5)
    ...
    if not IS_PARTNER:
        st.warning("Área restrita aos sócios.")
        st.stop()
    ...
    st.markdown("""
    <div class="rh-alert">
      <b>Segurança:</b> no Streamlit Cloud, configure usuários e senhas em <code>Secrets</code>.
